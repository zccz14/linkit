...
var _getElementAtPoint = require("./getElementAtPoint");
...
function useSwipeDismiss(options) {
  ...
  const setSwiping = (0, _useStableCallback.useStableCallback)(nextSwiping => {
  ...
  const updateSwipeProgress = (0, _useStableCallback.useStableCallback)((progress, details) => {
    const nextProgress = Number.isFinite(progress) ? (0, _clamp.clamp)(progress, 0, 1) : 0;
  ...
  const syncDragStyles = (0, _useStableCallback.useStableCallback)(swiping => {
    const element = elementRef.current;
  ...
  function getTargetAtPoint(position, nativeEvent) {
    ...
    const elementAtPoint = (0, _getElementAtPoint.getElementAtPoint)(root, position.x, position.y);
    const target = elementAtPoint ?? (0, _utils.getTarget)(nativeEvent);
    return target;
  }
  function findGestureScrollableTouchTarget(target, root) {
    if (hasHorizontal && !hasVertical) {
      return (0, _scrollable.findScrollableTouchTarget)(target, root, 'horizontal');
    }
    if (hasVertical && !hasHorizontal) {
      return (0, _scrollable.findScrollableTouchTarget)(target, root, 'vertical');
    }
    return (0, _scrollable.findScrollableTouchTarget)(target, root, 'vertical') ?? (0, _scrollable.findScrollableTouchTarget)(target, root, 'horizontal');
  }
  function startSwipeAtPosition(event, position, startOptions) {
    ...
    const doc = (0, _owner.ownerDocument)(elementRef.current);
    const body = doc.body;
    ...
    if (ignoreScrollableAncestors && element && target && scrollAxes.length > 0) {
      ...
      if (!ignoreAncestors && (0, _scrollable.hasScrollableAncestor)(target, element, scrollAxes)) {
        return false;
      }
    }
    cancelledSwipeRef.current = false;
  ...
  const handleStart = (0, _useStableCallback.useStableCallback)(event => {
    if (!enabled) {
      return;
    }
  ...
  const handleEnd = (0, _useStableCallback.useStableCallback)(event => {
    if (!enabled) {
      return;
    }
  ...
  const handleMove = (0, _useStableCallback.useStableCallback)(event => {
      ...
      if (allowedToStart) {
        ...
        if (isTouchLikeEvent(event)) {
          ...
          if (pendingStartPos && element) {
            ...
            if (scrollTarget && ((0, _utils.contains)(element, scrollTarget) || (0, _utils.contains)(scrollTarget, element))) {
  ...
  const moveNative = (0, _useStableCallback.useStableCallback)((nativeEvent, currentTarget) => {
    handleMove({
