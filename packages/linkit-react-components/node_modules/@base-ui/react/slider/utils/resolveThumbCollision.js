...
function resolveThumbCollision(behavior, values, currentValues, initialValues, pressedIndex, nextValue, min, max, step, minStepsBetweenValues) {
  ...
  if (behavior === 'push') {
    return {
      value: (0, _getPushedThumbValues.getPushedThumbValues)(activeValues, pressedIndex, nextValue, min, max, step, minStepsBetweenValues),
  ...
  const pressedValueAfterClamp = Number((0, _clamp.clamp)(nextValue, lowerBound, upperBound).toFixed(12));
    ...
    case 'swap':
      {
        ...
        const adjustedValues = (0, _getPushedThumbValues.getPushedThumbValues)(candidateValues, targetIndex, nextValueForTarget, min, max, step, minStepsBetweenValues, initialValuesForPush);
