import { createGridCellMap, getGridCellIndexOfCorner, getGridCellIndices, getGridNavigatedIndex, isListIndexDisabled } from "../../../floating-ui-react/utils/composite.mjs";
import { ARROW_DOWN, ARROW_LEFT, ARROW_RIGHT } from "../composite.mjs";
/**
 ...
 */
export function gridNavigation(config) {
  const {
    cols,
    dense = false,
    itemSizes
  ...
  return state => {
    ...
    const cellIndex = getGridNavigatedIndex(cellMap.map(itemIndex => itemIndex != null ? elementsRef.current[itemIndex] : null), {
      event,
      orientation,
      loopFocus,
      onLoop,
