    ...
    return (0, _mergeProps.mergeProps)({
      ...
      onKeyDown(event) {
        ...
        if (isCurrentTarget && isCompositeItem && isSpaceKey) {
          ...
          if (!isNativeButton || isButton) {
            ...
            (0, _dispatchClickWithModifiers.dispatchClickWithModifiers)(currentTarget, event);
          }
          return;
        }
      ...
      onKeyUp(event) {
        ...
        if (event.target === event.currentTarget && !isNativeButton && !isCompositeItem && !event.defaultPrevented && event.key === ' ') {
          event.preventBaseUIHandler();
          (0, _dispatchClickWithModifiers.dispatchClickWithModifiers)(event.currentTarget, event);
        }
      },
      onPointerDown(event) {
        if (disabled) {
...
function isValidLinkElement(elem) {
  return (0, _dom.isHTMLElement)(elem) && elem.tagName === 'A' && Boolean(elem.href);
}